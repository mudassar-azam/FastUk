<?php
    ...
    function get_UserById1($id_user){
        include("connexion.php");
        $con->set_charset("utf8");
    ...
    function setTypeVehiculeRental($libelle,$prix,$image){
        include("connexion.php");
        $con->set_charset("utf8");
        $res = '0';
        $libelle = str_replace("'","\'",$libelle);
        ...
        // }else{
            ...
            $sql = "INSERT INTO tj_type_vehicule_rental (libelle,prix,image,creer) VALUES ('$libelle','$prix','$image','$date_heure')";
            $result = mysqli_query($con,$sql);
    ...
    function setTypeVehiculeRentalMod($id,$libelle,$prix,$image,$miles){
        include("connexion.php");
        $con->set_charset("utf8");
        $res = '0';
        $libelle = str_replace("'","\'",$libelle);
        ...
        $date_heure = date('Y-m-d H:i:s');
        if($image == null){
              $sql = "UPDATE tj_type_vehicule_rental SET miles='$miles',prix='$prix',libelle='$libelle', modifier='$date_heure' WHERE id=$id";
        }else{
              $sql = "UPDATE tj_type_vehicule_rental SET miles='$miles',image='$image',prix='$prix',libelle='$libelle', modifier='$date_heure' WHERE id=$id";
        }
...
function getUserBooking(){
    include("connexion.php");
    $con->set_charset("utf8");
    $sql = "SELECT b.id as booking_id,b.from_address,b.to_address,b.status,b.vehicle_id,b.booking_date,b.pick_time,b.coll_name,b.coll_phone,b.coll_address,b.coll_city,b.deli_name,b.deli_phone,b.deli_address,b.deli_city,b.price,c.numberplate as driver_name,u.name as user_name FROM user_bookings b, users u,tj_vehicule c WHERE b.user_id=u.id  AND b.vehicle_id=c.id
        ORDER BY b.id DESC";
    $result = mysqli_query($con,$sql);
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) {
...
function get_guestBooking(){
    include("connexion.php");
    $con->set_charset("utf8");
    $sql = "SELECT b.id as booking_id,b.from_address,b.to_address,b.status,b.vehicle_id,b.booking_date,b.pick_time,b.coll_name,b.coll_phone,b.coll_address,b.coll_city,b.deli_name,b.deli_phone,b.deli_address,b.deli_city,b.price,c.numberplate as driver_name,u.name as user_name FROM guest_bookings b, guests u,tj_vehicule c WHERE b.guest_id=u.track_id  AND b.vehicle_id=c.id
        ORDER BY b.id DESC";
    $result = mysqli_query($con,$sql);
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) {
