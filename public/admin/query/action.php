<?php
    ...
    if((isset($_GET['id_vehicule_active']))){

        $run=mysqli_query($con,"UPDATE tj_vehicule SET statut='yes' where id='".$_GET['id_vehicule_active']."'");
        $_SESSION['status'] = 1;
        header('Location: '.$_SERVER['HTTP_REFERER']);
    }
    if((isset($_GET['id_vehicule_deact']))){

        $run=mysqli_query($con,"UPDATE tj_vehicule SET statut='no' where id='".$_GET['id_vehicule_deact']."'");
        $_SESSION['status'] = 1;
        header('Location: '.$_SERVER['HTTP_REFERER']);
    }
    if((isset($_GET['id_vehicule_delete']))){

        $run=mysqli_query($con,"DELETE from tj_vehicule where id='".$_GET['id_vehicule_delete']."'");
        $_SESSION['status'] = 1;
        header('Location: '.$_SERVER['HTTP_REFERER']);
    }
    ...
    && (isset($_POST['price_vehicule_rental_mod']) && !empty($_POST['price_vehicule_rental_mod']))){
        ...
        $miles = $_POST['miles_vehicule_rental_mod'];
        if($_FILES['image_vehicule_rental_mod']['tmp_name'] != ''){
        ...
        // If all the checks are passed, file is uploaded
            ...
            if (move_uploaded_file($_FILES["image_vehicule_rental_mod"]["tmp_name"], $target_file)) {
                ...
                $res = setTypeVehiculeRentalMod($_POST['id_type_vehicule_rental_mod'],$_POST['libelle_type_vehicule_rental_mod'],$_POST['price_vehicule_rental_mod'],$newfilename,$miles);
                // if($res == 1)
                    $_SESSION['status'] = 1;
                    header('Location: '.$_SERVER['HTTP_REFERER']);
            } else {
             ...
             $res = setTypeVehiculeRentalMod($_POST['id_type_vehicule_rental_mod'],$_POST['libelle_type_vehicule_rental_mod'],$_POST['price_vehicule_rental_mod'],null,$miles);
